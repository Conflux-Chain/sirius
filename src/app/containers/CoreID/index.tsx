...
import { isCurrentNetworkAddress, isBase32Address } from 'utils';
  ...
  const validateAddress = (address, cb) => {
    if (isCurrentNetworkAddress(address)) {
      if (isBase32Address(address)) {
        ...
        setAddressFormatErrorMsg(t(translations.coreId.incorrectAddressType));
      }
    } else {
      setAddressFormatErrorMsg(t(translations.coreId.incorrectFormat));
    }
  };
