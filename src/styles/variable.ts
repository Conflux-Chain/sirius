export const sansSerifFont = `-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto,
  'Helvetica Neue', Helvetica, Arial, 'Noto Sans', sans-serif, 'Apple Color Emoji', 'Segoe UI Emoji',
  'Segoe UI Symbol', 'Noto Color Emoji'`;

export const monospaceFont = `'Roboto Mono', 'Lucida Console', Consolas, 'Liberation Mono', Menlo, Courier, monospace`;

export const blue0 = '#7789D3';
export const blue1 = '#26244B';
export const blue2 = '#4665f0';
export const blue3 = '#F5F7FF';
export const blue4 = '#7789D3';
export const gray0 = '#F1F3F9';
export const gray1 = '#FDFDFE';
export const gray2 = '#8A8C95';
export const gray3 = '#EBECED';
export const gray4 = '#74798C';
export const orange0 = '#F8963E';
export const black0 = '#0F1327';
